...
fn make_simple_cert() -> Result<rcgen::Certificate, rcgen::RcgenError> {
    let subject_alt_names = vec!["localhost".into()];
    let subject_alt_names = subject_alt_names
        .into_iter()
        .map(|s| rcgen::SanType::DnsName(s))
        .collect::<Vec<_>>();

    let not_before = rcgen::date_time_ymd(2022, 01, 01);
    let not_after = rcgen::date_time_ymd(2023, 01, 01);
...
async fn run(options: Opt) -> Result<()> {
    let (certs, key) = if let (Some(key_path), Some(cert_path)) = (&options.key, &options.cert) {
        ...
        let dirs = directories_next::ProjectDirs::from("rs", "strike", "archive-server").unwrap();
        ...
        let (cert, key) = match fs::read(&cert_path).and_then(|x| Ok((x, fs::read(&key_path)?))) {
            ...
            Err(ref e) if e.kind() == io::ErrorKind::NotFound => {
                ...
                let cert = make_simple_cert().unwrap();//rcgen::generate_simple_self_signed(vec!["localhost".into()]).unwrap();
